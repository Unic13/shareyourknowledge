// api/feedback.js
// POST /api/feedback
//      → public, learner submits feedback
//      body: { userId, subject, rating (1-5), message, chapter_id }
//
// GET  /api/feedback?subject=SQL
//      → public: aggregate + recent 10 for one subject
//        (used by the learner app)
//
// GET  /api/feedback?requesterId=1
//      → admin view: overall totals + per-subject breakdown,
//        scoped to the editor's mapped subjects
//        (admin/super_admin see everything)
//
// GET  /api/feedback?requesterId=1&subject=SQL
//      → admin view: recent feedback for one subject
...
async function getRequesterAndScope(requesterId) {
  ...
  const {
    admin_users_by_pk: requester,
  } = await hasuraRequest(GET_USER, {
    id: requesterId,
  });
  ...
  // Only editors are restricted by subject mappings.
  ...
  return {
    requester,
    allowedCodes: subject_content.map((s) => s.code),
  };
}
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (!r || r < 1 || r > 5) {
        return res.status(400).json({
          error: 'rating must be a number 1-5',
        });
      }
      ...
      return res.status(200).json({
        success: true,
        id: data.insert_feedback_one.id,
      });
    }
    ...
    if (req.method === 'GET') {
      ...
      if (!requesterId) {
        ...
        const agg =
          data.feedback_aggregate.aggregate;
      ...
      // Editor requested a subject they are not assigned to.
      if (
        subject &&
        allowedCodes &&
        !allowedCodes.includes(subject)
      ) {
      ...
      // Editor has no assigned subjects.
      if (
        !subject &&
        allowedCodes &&
        !allowedCodes.length
      ...
      // For the overall admin view, include per-subject stats.
      if (!subject) {
        const rowsData = await hasuraRequest(
          GET_FEEDBACK_ROWS,
          { where }
        );

        result.bySubject =
          groupBySubject(rowsData.feedback);
      }
